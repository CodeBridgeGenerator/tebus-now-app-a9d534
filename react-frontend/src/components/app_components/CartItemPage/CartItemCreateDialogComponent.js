...
const [userId, setUserId] = useState([])
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            id: _entity?.id,voucherId: _entity?.voucherId?._id,userId: _entity?.userId?._id,Quantity: _entity?.Quantity,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info CartItem updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in CartItem" });
        }
        setLoading(false);
    };
...
const userIdOptions = userId.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create CartItem" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="cartItem-create-dialog-component">
