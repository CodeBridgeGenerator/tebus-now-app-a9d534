...
import cartItemFakerFactory from "./cartItemFakerFactory";
...
const CartItemFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [voucherIdIds, setvoucherIdIds] = useState();
const [userIdIds, setuserIdIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("voucher")
                        .find({ query: { $select: ["_id","id"] } })
                        .then((data) => {
                        setvoucherIdIds(data.data);
                        });
client.service("users")
    ...
    const onRun = async () => {
        let fakeData = cartItemFakerFactory(props.user,fakerCount,voucherIdIds,userIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cartItem").create(elem));
...
export default connect(mapState, mapDispatch)(CartItemFakerDialogComponent);
